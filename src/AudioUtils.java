...
public class AudioUtils {
    ...
    private static TargetDataLine td;//目标数据行
    private static Info dataLineInfo;
    private static SourceDataLine sd;//源数据行
    ...
    public static TargetDataLine getTargetDataLine() throws LineUnavailableException {
        if (td != null) {//如果目标数据行非空
            return td;//返回
        } else {
            // 1.获取音频流数据
            // af为AudioFormat也就是音频格式
            af = getAudioFormat();
            info = new DataLine.Info(TargetDataLine.class, af);//用来专成目标格式
            // 这里的td实际上是
